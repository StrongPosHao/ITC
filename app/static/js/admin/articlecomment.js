...
function truedelete(){
    //此处需判断是单个删除还是批量删除
    if(cid){
        send_to_back_comment(cid)
        toastr.success('删除成功!')
    }
    else if(cids){
        if(cids.length > 0){
            send_to_back2_comment(cids)
        toastr.success('删除成功!')
        //一次请求之后及时清理数据
        cids = []
        }
...
//向后台发送Ajax请求,删除单个
function send_to_back_comment(commentId) {
    $.ajax({
...
//向后台发送Ajax请求,删除多个
function send_to_back2_comment(commentIds) {
    $.ajax({
...
//取消删除
function canceldelete_articlecom(){
    cid = undefined
    cids = []
}
